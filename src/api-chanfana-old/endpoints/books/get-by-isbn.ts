...
export class GetBookByISBN extends BendRoute {
  ...
  schema = {
    ...
    description:
      'Retrieves book metadata from Alexandria → Google Books → ISBNdb chain with circuit breaker protection',
    request: {
      params: z.object({
        isbn: z
          .string()
  ...
  async handle(c: AppContext) {
    ...
    try {
      ...
      if (!result || !result.success) {
        ...
        return c.json(
          createErrorResponse('Book not found', 404, ErrorCodes.NOT_FOUND, {}, c.req.raw),
          404,
        )
      }
