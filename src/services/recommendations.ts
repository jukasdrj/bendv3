...
import type { SimilarBook, SimilarBooksData, SubjectsData } from 'alexandria-worker'
...
export class RecommendationService {
  ...
  private async buildPreferenceVector(
    ...
    if (ratedBooks.length > 0) {
      const ids = ratedBooks.map((book) => book.work_key || book.isbn).filter(Boolean) as string[]
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success) {
          ...
          return data.data.results.filter((book) => !book.isbn || !excludeIsbn.includes(book.isbn))
        }
      }
    } catch (error) {
   ...
   */
  private calculatePreferenceScore(book: SimilarBook, preferences: UserPreferences): number {
    let score = 0
    ...
    if (book.publish_date) {
      ...
      const meetsMin =
        preferences.publication_year_min === null || year >= preferences.publication_year_min
      const meetsMax =
        preferences.publication_year_max === null || year <= preferences.publication_year_max
      if (meetsMin && meetsMax) {
        score += 5
      }
    }
