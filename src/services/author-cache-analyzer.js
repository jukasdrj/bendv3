...
export async function analyzeAuthorCacheDepth(authorName, env) {
  try {
    ...
    const extrapolatedAuthorCovers = Math.round(
      (authorCovers.length / sampleSize) * totalCovers,
    );
    ...
    const estimatedCoverage = Math.min(
      100,
      (extrapolatedAuthorCovers / typicalAuthorWorks) * 100,
    );
...
export async function getAuthorsNeedingExpansion(
  ...
  console.log(``);
  console.log(
    `📊 Cache Coverage Summary (${authorNames.length} authors analyzed):`,
  );
  console.log(
    `   ✅ Sufficient coverage (≥${coverageThreshold}%): ${authorNames.length - needingExpansion.length} authors`,
  );
  console.log(
    `   ⚠️  Needs expansion (<${coverageThreshold}%): ${needingExpansion.length} authors`,
  );
  console.log(``);
