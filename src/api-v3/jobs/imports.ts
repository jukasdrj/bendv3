        ...
        try {
          ...
          const pollInterval = setInterval(async () => {
            try {
              ...
              console.error('[V3 Import Stream] Poll error:', pollError)
              clearInterval(pollInterval)
              sendEvent('error', {
                jobId,
                error: {
          ...
          c.req.raw.signal.addEventListener('abort', () => {
            console.log(`[V3 Import Stream] Client disconnected: ${jobId}`)
            clearInterval(pollInterval)
            clearInterval(pingInterval)
            controller.close()
          })
        } catch (error: any) {
          console.error('[V3 Import Stream] Stream error:', error)
          sendEvent('error', {
            jobId,
            error: {
              code: 'INTERNAL_ERROR',
