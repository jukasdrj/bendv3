...
function calculateDerivedMetrics(metrics) {
  ...
  if (metrics.websocket) {
    ...
    derived.websocket = {
      avgConnectionDuration: totalConnections > 0
        ? (totalDuration / totalConnections / 1000).toFixed(2) + 's'
        : '0s',
  ...
  if (metrics.d1) {
    ...
    derived.d1 = {
      ...
      readWriteRatio: d1.writeQueries > 0
        ? ((d1.readQueries || 0) / d1.writeQueries).toFixed(2)
        : 'N/A',
      errorRate: totalQueries > 0
        ? ((d1.errorCount || 0) / totalQueries * 100).toFixed(2) + '%'
        : '0%',
    ...
    derived.externalApi = {
      googleBooks: {
        ...
        quotaPercentageUsed: ext.googleBooks?.quotaRemaining
          ...
          : '0%',
        errorRate: ext.googleBooks?.requestCount > 0
          ? ((ext.googleBooks.errorCount || 0) / ext.googleBooks.requestCount * 100).toFixed(2) + '%'
          : '0%'
      },
      isbndb: {
        ...
        quotaPercentageUsed: ext.isbndb?.quotaRemaining
          ...
          : '0%',
        errorRate: ext.isbndb?.requestCount > 0
      ...
      gemini: {
        ...
        errorRate: ext.gemini?.requestCount > 0
  ...
  if (metrics.cache) {
    ...
    derived.cache = {
      hitRate: totalReads > 0
        ? ((total.hits || 0) / totalReads * 100).toFixed(2) + '%'
        : '0%',
      churnRate: total.writes > 0
...
function assessHealth(metrics) {
  ...
  return {
    status: issues.length === 0 ? "healthy" : issues.some(i => i.severity === "error") ? "unhealthy" : "degraded",
    issues: issues,
  };
}
...
function formatPrometheus(metrics) {
  ...
  // Cache metrics
  if (metrics.cache && metrics.cache.total) {
  ...
  if (metrics.apiContract) {
    ...
    lines.push(`api_contract_validations_total{result="success"} ${(contract.totalValidations || 0) - (contract.validationFailures || 0)}`);
    lines.push(`api_contract_validations_total{result="failure"} ${contract.validationFailures || 0}`);
    lines.push('');
  }
  ...
  if (metrics.externalApi) {
    ...
    if (ext.googleBooks) {
      lines.push(`external_api_requests_total{provider="google_books"} ${ext.googleBooks.requestCount || 0}`);
    }
    if (ext.isbndb) {
    ...
    if (ext.googleBooks) {
      lines.push(`external_api_quota_remaining{provider="google_books"} ${ext.googleBooks.quotaRemaining || 0}`);
    }
    if (ext.isbndb) {
      lines.push(`external_api_quota_remaining{provider="isbndb"} ${ext.isbndb.quotaRemaining || 0}`);
    }
    lines.push('');
  }
  ...
  if (metrics.health) {
    ...
    const healthValue = metrics.health.status === 'healthy' ? 1 : metrics.health.status === 'degraded' ? 0.5 : 0;
