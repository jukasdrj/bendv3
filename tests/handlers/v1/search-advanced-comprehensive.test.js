...
import { mockGoogleBooksSearchResponse, mockGoogleBooksEmptyResponse, createMockFetchResponse } from '../../mocks/providers.js'
import { harryPotterBook, book1984, greatGatsbyBook } from '../../fixtures/books.js'
...
describe('GET /v1/search/advanced - Comprehensive', () => {
  ...
  describe('Multiple Parameter Combinations', () => {
    ...
    it('should search with title + author combination', async () => {
      ...
      // Verify both params were used in query
      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining(encodeURIComponent(book1984.title)),
        expect.any(Object)
      )
      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining(encodeURIComponent(book1984.authors[0].name)),
        expect.any(Object)
      )
    })
  ...
  describe('Empty Results Handling', () => {
    ...
    it('should include helpful message for empty results', async () => {
      ...
      const response = await handleSearchAdvanced('xyz123', 'abc456', mockEnv, mockCtx)
  ...
  describe('Malformed Query Parameters', () => {
    ...
    it('should handle undefined parameters', async () => {
      const response = await handleSearchAdvanced(undefined, undefined, mockEnv, mockCtx)
    ...
    it('should trim whitespace from parameters', async () => {
      ...
      const response = await handleSearchAdvanced('  test  ', '  author  ', mockEnv, mockCtx)
  ...
  describe('Provider Integration', () => {
    ...
    it('should handle timeout errors', async () => {
      global.fetch = vi.fn().mockRejectedValue(new Error('Fetch timeout after 5000ms'))
    ...
    it('should handle malformed provider responses', async () => {
      global.fetch = vi.fn().mockResolvedValue(
        createMockFetchResponse({ incomplete: 'data' })
      )
