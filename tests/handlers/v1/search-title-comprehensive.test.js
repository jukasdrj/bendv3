...
describe('GET /v1/search/title - Comprehensive', () => {
  ...
  describe('Valid Title Queries', () => {
    ...
    it('should handle long titles', async () => {
      const longTitle = 'Harry Potter and the Philosopher\'s Stone: A Novel for Young Readers'
    ...
    it('should handle titles with special characters', async () => {
      ...
      expect(response.success).toBe(true)
      // Verify special chars were properly URL encoded
  ...
  describe('Provider Error Handling', () => {
    ...
    it('should handle empty results gracefully', async () => {
      ...
      const response = await handleSearchTitle('nonexistent book xyz123', mockEnv)
  ...
  describe('Response Envelope Format', () => {
    ...
    it('should include meta.source', async () => {
      const response = await handleSearchTitle(book1984.title, mockEnv)

      if (response.success) {
        expect(response.meta.source).toBeDefined()
        expect(typeof response.meta.source).toBe('string')
      }
    })
  })
  ...
  describe('Pagination Support', () => {
    ...
    it('should support maxResults parameter', async () => {
      const response = await handleSearchTitle(harryPotterBook.title, mockEnv, 10)

      if (response.success) {
        // Should limit results
        const totalResults = response.data.works.length +
