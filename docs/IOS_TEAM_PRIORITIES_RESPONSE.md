...
**CLARIFICATION:** BooksTrack uses a **hybrid SSE + WebSocket** strategy, similar to best-in-class apps like Slack, Discord, and GitHub. **WebSocket is NOT being deprecated for all use cases.**

### Transport Selection by Use Case

| Feature | Transport | Rationale | iOS Action |
|---------|-----------|-----------|------------|
| **CSV Import** | SSE | One-way updates, automatic reconnection, HTTP/2 friendly | ✅ Already using SSE - no change |
| **Batch Enrichment** | WebSocket | Bidirectional (potential future cancellation), real-time progress | ✅ Continue using WebSocket |
| **Bookshelf Scan** | WebSocket | Bidirectional (approval flow), complex job state | ✅ Continue using WebSocket |
| **Semantic Search** | HTTP REST | Request/response only, no streaming needed | ✅ No change needed |

### Why Hybrid?

**SSE is optimal for:**
- ✅ **One-way server→client updates** (CSV parsing progress)
- ✅ **Automatic reconnection** with `Last-Event-ID` (browser-native)
- ✅ **HTTP/2 multiplexing** (lower connection overhead)
- ✅ **No authentication needed** (jobId is sufficient)
- ✅ **Firewall-friendly** (plain HTTP, no upgrade handshake)

**WebSocket is optimal for:**
- ✅ **Bidirectional communication** (client can send cancellation, approval, etc.)
- ✅ **Complex state machines** (bookshelf scan approval flow)
- ✅ **Real-time updates** with minimal latency (<50ms)
- ✅ **Binary data** (future: stream book covers during enrichment)
- ✅ **Full-duplex** (server can push while client sends)

**Examples from Best-in-Class Apps:**
- **Slack:** WebSocket for messages (bidirectional), SSE for notifications
- **GitHub:** WebSocket for Copilot (interactive), SSE for Actions logs (one-way)
- **Discord:** WebSocket for voice/chat (low latency), REST for API calls

### Current Implementation Status

**API_CONTRACT.md §8 Deprecation Notice Applies To:**
- ❌ **NOT deprecating:** WebSocket for batch enrichment and bookshelf scan (bidirectional use cases)
- ⚠️ **Deprecated:** Using WebSocket for CSV import V1 (replaced by SSE in V2)

**Timeline:**

| Date | Milestone | Impact on iOS |
|------|-----------|---------------|
| **Nov 21, 2025** | SSE launched for CSV Import V2 | ✅ iOS CSVImportService already migrated to SSE |
| **Current** | WebSocket for batch enrichment & bookshelf scan | ✅ **NO MIGRATION NEEDED** - WebSocket stays |
| **Future (TBD)** | WebSocket enhancements (binary data, approval flow) | ✅ iOS benefits from improved WebSocket features |

### Action Items for iOS

- [ ] **Immediate:**
  - ✅ Verify `CSVImportService` uses SSE (no WebSocket)
  - ✅ Verify `BatchEnrichmentService` uses WebSocket (DO NOT migrate to SSE)
  - ✅ Verify `BookshelfScanService` uses WebSocket (DO NOT migrate to SSE)
  - ⚠️ Update TTL handling to 2 hours (was 1 hour)

- [ ] **Testing:**
  - Test SSE reconnection for CSV import (disconnect during job, verify `Last-Event-ID` works)
  - Test WebSocket reconnection for batch enrichment (network interruption recovery)
  - Test WebSocket late-connect for bookshelf scan (connect after job starts)
...
Response:
{
  ...
  "data": {
    ...
    "authToken": "ws-token-uuid",
    "websocketUrl": "/ws/progress?jobId=batch_enrich_uuid",
...
- Batch enrichment may support cancellation in the future (bidirectional)
- Consistent with bookshelf scan (both use WebSocket)
- SSE reserved for one-way updates (CSV import only)
...
- Create `/api/v2/books/enrich-batch` with WebSocket progress
...
- Publish WebSocket best practices guide

**Phase 3 (March 2026):**
- Sunset V1 batch endpoints (90-day notice: December 1, 2025)
- Enhance WebSocket with approval flow and binary data support
...
- [ ] **December 2025:**
  ...
  - Migrate `BatchEnrichmentService` to V2 (keep WebSocket)
...
**Priority:** P2 (Medium)
**Target:** Sprint 4 (December 2025)
**Description:** Create `/api/v2/books/enrich-batch` with WebSocket progress
**Requirements:**
- Same request format as `/api/batch-enrich`
- WebSocket progress (bidirectional, supports future cancellation)
- Deprecate `/api/batch-enrich` (90-day notice)
- Consistent with V2 response envelope

### Issue #TBD-3: WebSocket Best Practices Guide
**Priority:** P3 (Low)
**Target:** Sprint 4 (December 2025)
**Description:** Create `docs/WEBSOCKET_BEST_PRACTICES.md` for iOS team
**Requirements:**
- Code examples (Swift WebSocket implementation)
- Reconnection strategies for network interruptions
- Late-connect handling (connect after job starts)
- Authentication token security (prefer `Sec-WebSocket-Protocol` header)
...
|------|-----------|------------|
| **Nov 27, 2025** | Current state (hybrid SSE + WebSocket stable) | ✅ Verify TTL handling (2 hours) |
| **Dec 15, 2025** | Semantic search auth launched | Test authenticated semantic search |
| **Dec 31, 2025** | V2 batch enrichment endpoint (with WebSocket) | Test new endpoint, keep WebSocket |
| **Jan 15, 2026** | WebSocket best practices guide published | Review guide, improve reconnection logic |
| **Q2 2026** | WebSocket enhancements (approval flow, binary data) | Test new WebSocket features |
