...
- **📋 Sprint 3:** Backend Foundation + V2 API **[PLANNED - REVISED]**
  - **Vectorize Infrastructure:** Provision index, Workers AI binding (BGE-M3)
  - **D1 Schema:** Books table, migration from KV
  - **Unified V2 API Endpoints:**
  ...
  - **Cron Job:** Weekly recommendation generation (Gemini)
  - **Target:** January 2026
  - **Docs:** See [SPRINT_3_ORCHESTRATION.md](SPRINT_3_ORCHESTRATION.md), [API_CONTRACT_V2_PROPOSAL.md](../../API_CONTRACT_V2_PROPOSAL.md)

- **📋 Sprint 4:** Intelligence v2 - iOS Integration **[PLANNED - REVISED]**
  - **iOS SSE Client:** Native `SSEClient` actor for CSV import progress
  - **Semantic Search UI:** Natural language query support
  - **Recommendations Widget:** Weekly picks display
  - **V2 API Integration:** Migrate from WebSocket to HTTP/SSE
  - **End-to-End Testing:** Full flow validation
  - **Target:** February 2026
  - **Docs:** See [SPRINT_4_INTELLIGENCE_V2.md](SPRINT_4_INTELLIGENCE_V2.md)
...
- 📋 Weekly book recommendations (global, non-personalized)
...
### 📋 Sprint 3: Backend Foundation + V2 API (January 2026)

**Focus:** Unified V2 API infrastructure for Intelligence features

**✅ Backend Infrastructure Complete (Nov 25, 2025):**
- [x] Provision Cloudflare Vectorize index (1024 dims, cosine) - `book-embeddings`
- [x] Add Workers AI binding (BGE-M3 model) - `AI` binding
- [x] D1 schema for books table - migrations 0001-0008 applied
- [x] KV cache for recommendations - `RECOMMENDATIONS_CACHE`
- [x] Enrichment queue - `enrichment-queue`
- [x] Weekly cron trigger - Sunday midnight UTC
- [x] Embedding service - `src/services/embedding-service.ts`
- [x] Semantic search handlers - `src/handlers/semantic-search-handler.ts`
- [x] Gemini API key in secrets store

**🔧 Endpoint Implementation Remaining:**
- [x] `GET /v1/search/semantic` - **Live** (V1 namespace)
- [x] `GET /v1/search/similar` - **Live** (V1 namespace)
- [ ] `GET /api/v2/search?mode=text|semantic` - V2 wrapper
- [ ] `POST /api/v2/books/enrich` - Sync enrichment
- [ ] `POST /api/v2/imports` + SSE stream
- [ ] `GET /api/v2/recommendations/weekly` - Cron handler
- [ ] `GET /api/v2/capabilities` - Feature discovery

**Exit Criteria:**
- All V2 endpoints deployable to staging
- Semantic search returns relevant results (infrastructure ready ✅)
- Weekly recommendations generated via cron
- API contract finalized and documented

**Docs:** [SPRINT_3_BACKEND_HANDOFF.md](SPRINT_3_BACKEND_HANDOFF.md), [API_CONTRACT_V2_PROPOSAL.md](API_CONTRACT_V2_PROPOSAL.md)

---

### 📋 Sprint 4: Intelligence Live (February 2026)

**Focus:** iOS integration + feature launch

**iOS Tasks:**
- [ ] Implement native `SSEClient` actor (Swift 6)
- [ ] Build semantic search UI (SearchView enhancements)
- [ ] Create recommendations widget (HomeView)
- [ ] Migrate CSV import from WebSocket to SSE
- [ ] Update network layer for V2 API
- [ ] End-to-end testing (import → search → recommend)
...
**Docs:** [SPRINT_4_INTELLIGENCE_V2.md](SPRINT_4_INTELLIGENCE_V2.md)
